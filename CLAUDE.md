...
**Appel & Jacobson move generation.** The computer engine and the `find_words` MCP tool both use the same code in `supabase/functions/_shared/moveGenerator.ts`. It walks anchor squares with cross-check sets and generates every legal play in milliseconds. Move *selection* (which of those legal plays the computer plays) lives in `_shared/computerStrategy.ts`. Two strategies, one `strength` slider for each: `percentile` plays the move at rank `floor((100 − strength) × (N − 1) / 100)` of the score-sorted list (Easy = 90, Medium = 95, Hard = 100; slider exposes 90–100); `dynamic` targets the leader's running score with `target = gap + (strength/100) × leaderAvgMove` (Competitive = 100). Custom strengths in between are exposed via the create-game form's slider.
